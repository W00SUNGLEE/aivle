...
-- ROW_NUMBER():
SELECT ROW_NUMBER() OVER(ORDER BY e.emp_name ASC) as num,
	   e.emp_name, e.emp_id, e.gender, e.dept_id, d.dept_name, e.hire_date, e.email
	from employee as e
    inner join department as d on e.dept_id = d.dept_id
    where e.retire_date is null;
    
SELECT e.emp_name, e.emp_id, e.gender, e.dept_id, d.dept_name, e.hire_date, e.email, e.salary,
	   RANK() OVER(partition by gender ORDER BY e.salary DESC) as salary_rank
	from employee as e
    inner join department as d on e.dept_id = d.dept_id
    where e.retire_date is null;
  
SELECT e.emp_name, e.emp_id, e.gender, e.dept_id, d.dept_name, e.salary,
...
SELECT e.emp_name, e.emp_id, e.gender, e.dept_id, d.dept_name, e.salary,
	   NTILE(3) OVER(ORDER BY e.salary DESC) as salary_rank
	from employee as e
    inner join department as d on e.dept_id = d.dept_id
    where e.retire_date is null;
    
SELECT e.emp_name, e.emp_id, e.gender, e.dept_id, d.dept_name, e.salary,
	   ELT(NTILE(3) OVER(ORDER BY e.salary DESC), '상', '중', '하') as salary_rank
	from employee as e
    inner join department as d on e.dept_id = d.dept_id
    where e.retire_date is null;
    
select emp_name, dept_id hire_date, salary,
	   sum(salary) over(partition by dept_id order by hire_date) as total_salary,
	   avg(salary) over(partition by dept_id order by hire_date) as avg_salary
	from employee
    where salary > 0
